...
from utils import calibrate, calibrate_v0, crop, resize, exception_to_str, get_calibration_frame_dist, \
    get_extension_agnostic_path, multi_file_extension_glob, blur_and_downsample
...
def run(config: Config):
    ...
    assert os.path.isdir(os.path.join(config.data_dir, "transects")) and os.path.isdir(os.path.join(config.data_dir,
                                                                                                    "results")), "Data dir must contain 'transect' and 'results' subdirectories. Please consult the manual for the correct directory structure."
    assert len(glob.glob(os.path.join(config.data_dir, "transects",
                                      "*/"))), "The 'transect' subdirectory must contain at least one transect. Please consult the manual for the correct directory structure."
    ...
    elif config.depth_estimation_model == DepthEstimationModel.UniDepthv2:
        depth_estimation_model = UniDepthv2()
    elif config.depth_estimation_model == DepthEstimationModel.DepthPro:
    ...
    with open(os.path.join(config.data_dir, "results", "results.csv"), "w", newline="") as result_csv_file, open(
            os.path.join(config.data_dir, "results", "results.txt"), "w") as result_distance_file:
        head_row_csv = ["transect_id", "frame_id", "detection_idx", "detection_confidence", "depth", "world_x",
                        "world_y", "world_z"]
        ...
        for transect_idx, transect_dir in enumerate(transect_dirs):
            ...
            if config.depth_estimation_model != DepthEstimationModel.DEPTH_AHYTHING_METRIC:
                for calibration_frame_filename in (
                        multi_file_extension_glob(os.path.join(transect_dir, "calibration_frames", "*"),
                                                  config.intensity_image_extensions) +
                        multi_file_extension_glob(os.path.join(transect_dir, "calibration_frames_cropped", "*"),
                                                  config.intensity_image_extensions)
                        # for backwards compability. use crop configuration instead
                ):
                    yield
                ...
                farthest_calibration_frame_disp = list(calibration_frames.values())[-1] if len(
                    calibration_frames) > 0 else None
                ...
                try:
                    ...
                    for dist, disp in calibration_frames.items():
                        ...
                        result_distance_file.write("\t".join([str(dist),
                                                             str(np.median(disp_calibrated.data[disp_calibrated.mask])),
                                                             str(dist ** -1)]) + os.linesep)

                    calibration = do_calibrate(np.array(x) ** exp, np.array(y) ** exp,
                                               config.calibration_regression_method)
                    ...
                    # save into file calibration points and plot
                    ...
                    if not os.path.exists(os.path.join(transect_dir, "detection_frames_depth")):
                        logging.warn(
                            f"Failed calibrating transect '{transect_id}' due to exception: {exception_to_str(e)}. Skipping all distance estimations for observations in this transect.")

                yield

                if config.make_figures and farthest_calibration_frame_disp is not None:
                    visualize_farthest_calibration_frame(config.data_dir, transect_id, farthest_calibration_frame_disp,
                                                         config.min_depth, config.max_depth)

            detection_frame_filenames = sorted(list(set(
                multi_file_extension_glob(os.path.join(transect_dir, "detection_frames", "*"),
                                          config.intensity_image_extensions) +
                multi_file_extension_glob(os.path.join(transect_dir, "detection_frames_cropped", "*"),
                                          config.intensity_image_extensions)
                # for backwards compability. use crop configuration instead
            )))
            for detection_idx, detection_frame_filename in enumerate(detection_frame_filenames):
                ...
                yield StatusUpdate(transect_id, transect_idx, len(transect_dirs), detection_id, detection_idx,
                                   len(detection_frame_filenames))
                ...
                scores, labels, boxes = scores[high_confidence_idx], labels[high_confidence_idx], boxes[
                    high_confidence_idx]

                # sort from image center outwards
                centerness = [((img.shape[1] / 2) - (box[0] + box[2]) / 2) ** 2 + (
                        (img.shape[0] / 2) - (box[1] + box[3]) / 2) ** 2 for box in boxes]
                ...
                if config.detection_sampling_method == DetectionSamplingMethod.SAM:
                    ...
                    for box in boxes:
                        ymin, ymax = max(0, min(img.shape[0] - 2, round(box[1]))), max(0, min(img.shape[0] - 1,
                                                                                              round(box[3])))
                        xmin, xmax = max(0, min(img.shape[1] - 2, round(box[0]))), max(0, min(img.shape[1] - 1,
                                                                                              round(box[2])))
                ...
                if config.depth_estimation_model == DepthEstimationModel.DEPTH_AHYTHING_METRIC:
                    ...
                    precomputed_depth_filename = get_extension_agnostic_path(
                        os.path.join(transect_dir, "detection_frames_depth", detection_id),
                        config.depth_image_extensions)
                    if precomputed_depth_filename is None and farthest_calibration_frame_disp is None:
                        logging.warn(
                            f"Unable to perform distance estimation on detection '{detection_id}' due to failed calibration and no precomputed depth maps.")
                        continue
                        ...
                        if config.sample_from == SampleFrom.DETECTION:
                            ...
                            if config.calibrate_blur:
                                disp = do_calibrate(blur_and_downsample(disp_masked),
                                                    blur_and_downsample(farthest_calibration_frame_disp) ** exp,
                                                    config.calibration_regression_method)(disp ** exp)
                            else:
                                disp = do_calibrate(disp_masked, farthest_calibration_frame_disp ** exp,
                                                    config.calibration_regression_method)(disp ** exp)
                            if config.calibrate_metric:
                ...
                for box, mask in zip(boxes, masks):
                    ...
                    if config.detection_sampling_method == DetectionSamplingMethod.BBOX_BOTTOM:
                        ...
                        ymin, ymax = max(0, min(depth.shape[0] - 2, round(box[1]))), max(0, min(depth.shape[0] - 1,
                                                                                                round(box[3])))
                        xmin, xmax = max(0, min(depth.shape[1] - 2, round(box[0]))), max(0, min(depth.shape[1] - 1,
                                                                                                round(box[2])))
                        depth_cropped = depth[ymin:ymax, xmin:xmax]
                        sampled_depths += [
                            np.percentile(depth_cropped, config.bbox_sampling_percentile, method="nearest")]
                        ...
                        dist = cv2.distanceTransform((mask_paddded * 255).astype(np.uint8), cv2.DIST_L2,
                                                     cv2.DIST_MASK_3)
                        ...
                        raise RuntimeError(
                            f"Invalid configuration value '{config.detection_sampling_method}' for configuration detection_sampling_method")
                ...
                for i, (score, sampled_depth, world_position) in enumerate(
                        zip(scores, sampled_depths, world_positions)):
                    detection_i = i if config.multiple_animal_reduction != MultipleAnimalReduction.MEDIAN else -1
                    result_csv_writer.writerow([transect_id, detection_id, f"{detection_i:03d}", f"{score.item():.4f}",
                                                f"{sampled_depth.item():.4f}", f"{world_position[0].item():.4f}",
                                                f"{world_position[1].item():.4f}", f"{world_position[2].item():.4f}"])
